...
<?php
    ...
    if(isset($_GET['id'])) {
        $id = $_GET['id'];
        $query = "DELETE FROM `Universitate` WHERE id_universitate=$id";
        $query_run=mysqli_query($link,$query);
        if($query_run)
        {
            echo '<script type="text/javascript"> alert("Datele au fost sterse cu succes!")</script>';
        }
        else
        {
            echo '<script type="text/javascript"> alert("Datele nu au fost sterse")</script>';
        }
    }
?>
